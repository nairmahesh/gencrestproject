...
const MobileApp: React.FC<MobileAppProps> = ({ children }) => {
  ...
  // Sample data for liquidation
  const liquidationData = [
    {
      id: 'DIST001',
      distributorName: 'SRI RAMA SEEDS',
      distributorCode: 'SRS001',
      ...
      metrics: {
        openingStock: { volume: 50, value: 0.60 },
        ytdNetSales: { volume: 350, value: 1.75 },
        liquidation: { volume: 150, value: 0.85 },
        balanceStock: { volume: 250, value: 1.50 },
    ...
    },
    {
      ...
      metrics: {
        openingStock: { volume: 80, value: 0.96 },
        ytdNetSales: { volume: 420, value: 2.10 },
        liquidation: { volume: 200, value: 1.20 },
        balanceStock: { volume: 300, value: 1.86 },
    ...
    },
    {
      ...
      metrics: {
        openingStock: { volume: 100, value: 1.20 },
        ytdNetSales: { volume: 480, value: 2.40 },
        liquidation: { volume: 250, value: 1.50 },
        balanceStock: { volume: 350, value: 2.10 },
        liquidationPercentage: 25,
        lastUpdated: new Date().toISOString()
      }
    }
  ...
  const retailerData = [
    ...
    {
      id: 'RET002',
      distributorName: 'Farm Fresh Supplies',
      distributorCode: 'FFS001',
      territory: 'Market Area',
      region: 'Delhi NCR',
      zone: 'North Zone',
      status: 'Active' as const,
      ...
      metrics: {
        openingStock: { volume: 40, value: 0.50 },
        ytdNetSales: { volume: 220, value: 1.10 },
        liquidation: { volume: 140, value: 0.75 },
        balanceStock: { volume: 120, value: 0.85 },
        liquidationPercentage: 54,
        lastUpdated: new Date().toISOString()
      }
    }
  ];
  ...
  const allEntities = [...liquidationData, ...retailerData];
  ...
  const getTagColor = (tag: string) => {
    if (tag === 'Distributor') return 'bg-purple-100 text-purple-800';
    if (tag === 'Retailer') return 'bg-blue-100 text-blue-800';
  ...
  const renderLiquidationTab = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white">
        <h2 className="text-xl font-bold mb-2">Liquidation Tracking</h2>
        <p className="text-purple-100 text-sm">Monitor distributor & retailer performance</p>
      </div>
      ...
      <div className="p-4 bg-white border-b space-y-4">
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search filters (Distributor, Retailer, etc.)..."
              value={searchTag}
          ...
          {selectedTags.length > 0 && (
            <button
              ...
              className="px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors flex items-center text-sm"
            >
              <X className="w-4 h-4" />
            </button>
          )}
          ...
          <div className="space-y-3">
            ...
            {otherTags.filter(tag => tag.toLowerCase().includes(searchTag.toLowerCase())).length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {otherTags.filter(tag => tag.toLowerCase().includes(searchTag.toLowerCase())).slice(0, 8).map(tag => (
                    <button
                      key={tag}
                      onClick={() => handleTagToggle(tag)}
          ...
          <div key={item.id} className="p-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  item.tags.includes('Distributor') ? 'bg-purple-100' : 'bg-blue-100'
                }`}>
                  <Building className={`w-5 h-5 ${
                    item.tags.includes('Distributor') ? 'text-purple-600' : 'text-blue-600'
                  }`} />
                </div>
                <div>
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-purple-600">{item.metrics.liquidationPercentage}%</div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-orange-50 rounded-lg p-3 text-center">
                <div className="text-xs text-orange-600 font-medium mb-1">Opening Stock</div>
                <div className="font-bold text-orange-800">{item.metrics.openingStock.volume}</div>
                <div className="text-xs text-orange-600">₹{item.metrics.openingStock.value}L</div>
              </div>
              <div className="bg-blue-50 rounded-lg p-3 text-center">
                <div className="text-xs text-blue-600 font-medium mb-1">YTD Sales</div>
                <div className="font-bold text-blue-800">{item.metrics.ytdNetSales.volume}</div>
                <div className="text-xs text-blue-600">₹{item.metrics.ytdNetSales.value}L</div>
              </div>
              <div className="bg-green-50 rounded-lg p-3 text-center">
                <div className="text-xs text-green-600 font-medium mb-1">Liquidation</div>
                <div className="font-bold text-green-800">{item.metrics.liquidation.volume}</div>
                <div className="text-xs text-green-600">₹{item.metrics.liquidation.value}L</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-3 text-center">
                <div className="text-xs text-purple-600 font-medium mb-1">Balance</div>
                <div className="font-bold text-purple-800">{item.metrics.balanceStock.volume}</div>
                <div className="text-xs text-purple-600">₹{item.metrics.balanceStock.value}L</div>
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{ width: `${item.metrics.liquidationPercentage}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="flex space-x-2">
              <button className="flex-1 bg-purple-100 text-purple-700 py-2 px-3 rounded-lg text-xs font-medium">
                Track Details
              </button>
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'liquidation': return renderLiquidationTab();

...
const Liquidation: React.FC = () => {
  ...
  const getModalData = (type: string, distributor: any) => {
    switch (type) {
      case 'opening':
        return {
          title: `Opening Stock - ${distributor.name}`,
          subtitle: 'SKU-wise opening stock breakdown',
          totalVolume: distributor.openingStock.volume,
          totalValue: distributor.openingStock.value,
          data: [
            { sku: 'DAP-25KG', skuName: 'DAP 25kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, salesValue: 0.14 },
              { invoice: 'INV-2024-002', date: '1/15/2024', batch: 'BATCH-002', currentStock: 105, salesValue: 0.14 }
            ]},
            { sku: 'DAP-50KG', skuName: 'DAP 50kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, salesValue: 0.14 }
            ]}
          ]
        };
      case 'ytd':
        return {
          title: `YTD Net Sales - ${distributor.name}`,
          subtitle: 'SKU-wise sales performance',
          totalVolume: distributor.ytdNetSales.volume,
          totalValue: distributor.ytdNetSales.value,
          data: [
            { sku: 'DAP-25KG', skuName: 'DAP 25kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, salesValue: 0.14 },
              { invoice: 'INV-2024-002', date: '1/15/2024', batch: 'BATCH-002', currentStock: 105, salesValue: 0.14 }
      ...
      case 'liquidation':
        return {
          title: `Liquidation - ${distributor.name}`,
          subtitle: 'SKU-wise liquidation breakdown',
          totalVolume: distributor.liquidation.volume,
          totalValue: distributor.liquidation.value,
          data: [
            { sku: 'DAP-25KG', skuName: 'DAP 25kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, salesValue: 0.14 },
              { invoice: 'INV-2024-002', date: '1/15/2024', batch: 'BATCH-002', currentStock: 105, salesValue: 0.14 }
      ...
      case 'balance':
        return {
          title: `Balance Stock - ${distributor.name}`,
          subtitle: 'SKU-wise stock verification',
          totalVolume: distributor.balanceStock.volume,
          totalValue: distributor.balanceStock.value,
          showVerifyStock: true,
          data: [
            { sku: 'DAP-25KG', skuName: 'DAP 25kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, physicalStock: 105 },
              { invoice: 'INV-2024-002', date: '1/15/2024', batch: 'BATCH-002', currentStock: 105, physicalStock: 105 }
            ]},
            { sku: 'DAP-50KG', skuName: 'DAP 50kg Bag', invoices: [
              { invoice: 'INV-2024-001', date: '1/15/2024', batch: 'BATCH-001', currentStock: 105, physicalStock: 105 }
            ]}
                ...
                <div className="grid grid-cols-4 gap-4 mb-4">
                  <div className="bg-orange-50 rounded-lg p-4 border-l-4 border-orange-500">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button 
                        onClick={() => setSelectedModal(`opening-${distributor.id}`)}
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button 
                        onClick={() => setSelectedModal(`ytd-${distributor.id}`)}
                  ...
                  <div className="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button 
                        onClick={() => setSelectedModal(`liquidation-${distributor.id}`)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
            {(() => {
              ...
              const modalData = getModalData(type, distributor);
              
              if (!modalData || !distributor) return null;
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="text-center bg-gray-50 rounded-xl p-6">
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">
                          {type === 'opening' ? 'Total Opening Stock' :
                           type === 'ytd' ? 'Total YTD Net Sales' :
                           type === 'liquidation' ? 'Total Liquidation' :
                           'Total Balance Stock'}
                        </h4>
                        <div className="grid grid-cols-2 gap-8">
                          <div>
                            <div className="text-4xl font-bold text-gray-900">{modalData.totalVolume}</div>
                            <div className="text-sm text-gray-600">Total Volume (Kg/Litre)</div>
                          </div>
                          <div>
                            <div className="text-4xl font-bold text-gray-900">₹{modalData.totalValue}L</div>
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">Invoice-wise Breakdown</h4>
                        
                        {modalData.data.map((skuData: any, skuIndex: number) => (
                          <div key={skuIndex} className="mb-6">
                            <div className="flex items-center space-x-3 mb-4">
                              <span className={`px-3 py-1 rounded-lg text-sm font-medium text-white ${
                                skuData.sku === 'DAP-25KG' ? 'bg-blue-600' : 'bg-green-600'
                              }`}>
                                {skuData.skuName}
                              </span>
                              <span className="text-sm text-gray-600">SKU: {skuData.sku}</span>
                            </div>
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                              <div className="grid grid-cols-4 gap-4 text-sm font-medium text-gray-700 mb-3">
                                ...
                                <div>SKU</div>
                                <div>
                                  {type === 'balance' ? 'Current Stock' : 
                                   type === 'ytd' ? 'Current Stock' : 'Opening Stock'}
                                </div>
                                <div>
                                  {type === 'balance' ? 'Physical Stock (Verified)' :
                                   type === 'ytd' ? 'Sales In Value (₹ Lakhs)' : 'Current Stock'}
                                </div>
                              </div>
                              
                              {skuData.invoices.map((invoice: any, invoiceIndex: number) => (
                                <div key={invoiceIndex} className="grid grid-cols-4 gap-4 items-center py-3 border-t border-gray-200">
                                  <div>
                                    <p className="font-medium text-gray-900">Invoice: {invoice.invoice}</p>
                                  ...
                                  <div className="text-center">
                                    <div className="bg-orange-100 rounded-lg p-3">
                                      <div className="text-lg font-bold text-orange-800">{invoice.currentStock}</div>
                                    </div>
                                  </div>
                                  <div className="text-center">
                                    {type === 'balance' ? (
                                      <div className="bg-gray-100 rounded-lg p-3">
                                        <input 
                                          type="number" 
                                          defaultValue={invoice.physicalStock || invoice.currentStock} 
                                          className="w-full text-center text-lg font-bold text-gray-800 bg-transparent border-none outline-none"
                                        />
                                      </div>
                                    ) : type === 'ytd' ? (
                                      <div className="bg-blue-100 rounded-lg p-3">
                                        <div className="text-lg font-bold text-blue-800">₹{invoice.salesValue}L</div>
                                      </div>
                                    ) : (
                                      <div className="bg-blue-100 rounded-lg p-3">
